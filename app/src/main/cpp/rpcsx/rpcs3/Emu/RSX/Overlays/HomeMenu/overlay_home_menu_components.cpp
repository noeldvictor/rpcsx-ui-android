...
namespace rsx
{
	namespace overlays
	{
		...
		home_menu_toggle_entry::home_menu_toggle_entry(const std::string& text, std::function<bool()> get_value)
			: m_get_value(std::move(get_value))
		{
			...
			m_last_value = m_get_value ? m_get_value() : false;
		}

		compiled_resource& home_menu_toggle_entry::get_compiled()
		{
			const bool new_value = m_get_value ? m_get_value() : false;
