...
- `app/src/main/cpp/rpcsx` is now a plain-file vendor copy of the upstream RPCSX core source. The full upstream Android core build entry is `app/src/main/cpp/rpcsx/android/CMakeLists.txt`; the regular Gradle app build still points at the lightweight wrapper CMake file, with experimental source-core packaging gated behind `RPCSX_BUILD_BUNDLED_CORE=1` / `-PbuildBundledRpcsxCore=true`.
- Source-core packaging checkpoint: the wrapper/core library names are separated (`librpcsx-ui-jni.so` for Java JNI, `librpcsx-android.so` for the bundled core), and the core Android CMake can now be included as a subdirectory. Configure still fails until the upstream third-party dependency trees are supplied; missing examples include `xbyak`, `SPIRV-Headers`, `SPIRV-Tools`, `Vulkan-Headers`, `SPIRV-Cross`, `glslang`, `json`, `LibAtrac9`, `yaml-cpp`, `cubeb`, `wolfssl`, `curl`, `fusion`, and `fmtlib`.
