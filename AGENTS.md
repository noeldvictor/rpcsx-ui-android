...
- 2026-05-16 Rocknix 26 FPS delta: local Rocknix `next` checkout packages `rpcs3-sa` as RPCS3 Linux ARM64 AppImage `0.0.40-19291-d773a3f9`. Matched Android Rocknix-style field tests did **not** close the gap: `RocknixCorrect` on stock Qualcomm hit about `18.49 FPS`, `RocknixFast` with WCB off hit about `19.17 FPS`, and Android Turnip A7xx dropped to about `12.37 FPS`. Treat the remaining gap to the reported Rocknix `26 FPS` as a runtime/system-driver/scheduler problem first, not a simple resolution/WCB/shader-precision settings miss. Details live in `debug-experiments/20260516-rocknix-26fps-delta.md`.
